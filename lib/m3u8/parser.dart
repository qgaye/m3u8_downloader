...
M3U8Key parseKey(String line) {
  ...
  line.split(':').last.split(',').forEach((value) {
    ...
    if (split.length != 2) {
      ...
      throw M3U8Exception('parse $EXT_X_KEY fail');
    }
    if (split[0] == 'METHOD') {
      key.method = split[1];
    } else if (split[0] == 'URI') {
